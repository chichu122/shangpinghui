  ...
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="(attr,index) in attrsList" :key="attr.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <!-- 平台售卖属性的属性值：粉色、蓝色、黑色。。。 -->
          <li v-for="(attrValue,index) in attr.attrValueList" :key="index" @click="attrInfo(attr,attr)">
            <a>{{attrValue}}</a>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    methods:{
      ...
      tradeMarkHandler(trademark){
        ...
        // 老师问题：在哪个组件中发请求，父组件？
        // 为什么？因为父组件中searchParams参数是带给服务器参数，子组件把你点击的品牌的信息，需要给父组件传递过去---用自定义事件
        // 触发自定义事件，事件名字、需要传给父组件的参数
