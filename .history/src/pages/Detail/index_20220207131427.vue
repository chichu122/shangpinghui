        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转：从A路由跳转到B路由，这里在加入购物车进行路由跳转之前，发请求
...
export default {
  ...
  methods:{
    ...
    async addShopcar(){
      ...
      /* 当前这里是派发一个action，也向服务器发请求，判断购物车是否加入成功，进行相应操作
         说白了this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid , skuNum:this.skuNum})
         这代码是在调用仓库中addOrUpdateShopCart方法，这个方法加上async，返回一定是一个promise
         2、要么成功要么失败,若成功路由跳转，若失败，需要给用户错误提示
      */
      ...
      try{
        ...
        // 下面这种手段路由跳转以及传递参数--可以，但由于传过去的产品信息是一个对象很复杂，会导致地址栏很丑，所以只传一个序号
        // this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}});
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum}});
        // 一些简单的数据skuNum，通过query形式给lu'you'zu'jian'chuan'di'guo'qu
      }catch(error){
        alert(error.message)
      }
