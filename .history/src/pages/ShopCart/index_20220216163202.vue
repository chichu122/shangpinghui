  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck&&cartInfoList.length>0" @change="updateAllCartIsChecked"/>
        <span >全选</span>
      </div>
      <div class="option">
      ...
      <div class="money-box">
        ...
        <div class="sumbtn">
          ...
          <router-link class="sum-btn" to='/trade'>jjie</router-link>
        </div>
      </div>
    </div>
  </div>
...
import throttle from "lodash/throttle"; //加入购物车数量节流，多次变少量
export default {
  name: "ShopCart",
  mounted() {
    this.getData();
  ...
  methods: {
    ...
    // 删除全部选中的产品
    // 这个回调函数因为结构问题，所以我们拿不到cart数据，即拿不到id
  ...
  computed: {
    ...
    isAllCheck() {
      // 遍历数组里面的元素，只要全部的元素isChecked属性都为1===>真true
      // 只要有一个不是1=======》假false
